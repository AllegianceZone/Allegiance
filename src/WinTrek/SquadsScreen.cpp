...
*/
    ...
    {
        ShellExecuteA(NULL, NULL, m_peditPaneSquadURL->GetString(), NULL, NULL, SW_SHOWNORMAL);
...
//              m_pSquadsScreen->OnSquadSelected(*psquad);
            ...
            {
                sprintf(cbTemp, "%i", psquad->GetRanking(SquadsScreen::s_column));
			...
			if (strcmp(PCC(psquad->GetName()), "Click Here to Enter Name") == 0)
                sprintf(cbTemp, "New Squad");
            else
                sprintf(cbTemp, "%s", psquad->GetName());
            ...
            // Display Score
            //
            sprintf(cbTemp, "%i", psquad->GetScore());
            psurface->DrawString(pfont, Color::White(), WinPoint(m_viColumns[1] + 2, 0), cbTemp);

            //
            // Display Wins
            //
            sprintf(cbTemp, "%i", psquad->GetWins());
            psurface->DrawString(pfont, Color::White(), WinPoint(m_viColumns[2] + 2, 0), cbTemp);

            //
            // Display Losses
            //
            sprintf(cbTemp, "%i", psquad->GetLosses());
...
//              m_pSquadsScreen->OnPlayerSelected(*pplayer);
            ...
            sprintf(cbTemp, "%s", pplayer->GetName());
            psurface->DrawString(pfont, Color::White(), WinPoint(/*m_viColumns[-1] + */2, 0), cbTemp);
            psurface->RestoreClipRect(rectClipOld);
            ...
            sprintf(cbTemp, "%s", PCC(trekClient.LookupRankName((RankID)pplayer->GetRank(), civid)));
            ...
            sprintf(cbTemp, "%s", IZonePlayer::DetailedStatus2String(detailedstatus));
            psurface->DrawString(pfont, Color::White(), WinPoint(m_viColumns[1] + 2, 0), cbTemp);
            psurface->RestoreClipRect(rectClipOld);
            ...
            if(s_pSelectedSquad)
            {
              sprintf(cbTemp, "%s", pplayer->GetLastPlayedDate(*s_pSelectedSquad));
              psurface->DrawString(pfont, Color::White(), WinPoint(m_viColumns[2] + 2, 0), cbTemp);
            }
        }
    };
