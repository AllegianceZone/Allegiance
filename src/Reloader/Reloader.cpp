...
void DisplayErrorMsg(char *szError)
{
    ...
    ::MessageBoxA(NULL, szError, sz, MB_ICONEXCLAMATION);
}
...
class CAutoDownloadSink : public IAutoUpdateSink
{
  ...
  bool OnMoveError(char *szErrorMessage)
  {
      ...
      if (++cCalled > 25) // 25 secs
      {
          ...
          if (::MessageBoxA(NULL, szMsg, szTitle, MB_YESNO) == IDYES)
              return true;
          else
              return false;
      }
...
void RenameMangledFile(char * szMangled, char * szCorrect)
{
        if (Exists(szMangled))
        {
            ...
            DeleteFileA("AutoUpdate\\FileList.txt");
            ...
            if (Exists(szCorrect))
            {
                ...
                if (CompareFileTime(&ftCorrect, &ftMangled) > 0) // if existing correct file is newer; keep it
                {
                    ...
                    OutputDebugStringA(szMangled);
                    DeleteFileA(szMangled);
                    return;
                }

                OutputDebugStringA("\nDeleting old file with correct name ");
                OutputDebugStringA(szCorrect);
                if (!DeleteFileA(szCorrect))
                {
                    ::Sleep(2000);
                    if (!DeleteFileA(szCorrect))
                    {
                        ::Sleep(10000);
                        DeleteFileA(szCorrect);
                    }
                }
            }
            ...
            if (!MoveFileA(szMangled, szCorrect))
            {
