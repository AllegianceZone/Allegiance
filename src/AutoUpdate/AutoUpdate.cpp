...
void MsgBox(bool bService, const char* format, ...)
{
	...
	if (bService) {
		...
		MessageBoxA(NULL, bfr, "AutoUpdate", MB_SERVICE_NOTIFICATION);
	}
}
...
public:
        ...
        if (!bErrorOccurred)
        {
            ...
            if (m_pdlg->m_szPostUpdateEXE[0] != 0)
            {
                ...
                if ((szEXE[0] == 'S' || szEXE[0] == 's') && szEXE[1] == ':')
                {
                  ...
                  if (hSCM)
                  {
                    SC_HANDLE hService = ::OpenServiceA(hSCM, szEXE, SERVICE_START);
    ...
    virtual void OnProgress(unsigned long cTotalBytes, const char* szCurrentFile, unsigned long cCurrentFileBytes, unsigned cEstimatedSecondsLeft) 
    {
        ...
        if (szCurrentFile != NULL && szCurrentFile != szPrevCurrentFile)
        {
			...
			//imago 10/14
			size_t newsize = strlen(szCurrentFile) + 1;
			wchar_t * wcstring = new wchar_t[newsize];
			size_t convertedChars = 0;
			mbstowcs_s(&convertedChars, wcstring, newsize, szCurrentFile, _TRUNCATE);
			m_pdlg->m_staticFileName.SetWindowText(wcstring);
        }
        else
        {
