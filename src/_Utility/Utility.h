...
class MMF 
{
	public:
			...
			m_uBuffer(0)
		{
			...
			if (bCreate)
			{
				m_MapHandle = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, m_iBuffer, m_szMapName);
			...
			{
				m_MapHandle = OpenFileMappingA(FILE_MAP_ALL_ACCESS, TRUE, m_szMapName);
				if (m_MapHandle != INVALID_HANDLE_VALUE)
					m_szBuffer = (char*)MapViewOfFile(m_MapHandle, FILE_MAP_ALL_ACCESS, 0, 0, m_iBuffer);
				else m_MapHandle = NULL;
			}
