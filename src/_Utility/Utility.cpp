                         ...
                         OUT char * szPath)
{
    ...
    szPath[0] = 0;
    LONG ret = RegOpenKeyExA(hkey, szSubKey, 0, KEY_READ, &hKey);
    
    if (ERROR_SUCCESS == ret)
    {
        ret = RegQueryValueExA(hKey, "ArtPath", NULL, &dw, (unsigned char*)szPath, &cb);
        RegCloseKey(hKey);
    }
    return ret;
}
