...
void FedMessaging::EnumHostsCallback ( const DPNMSG_ENUM_HOSTS_RESPONSE& resp )
{
	...
	sprintf(szBuff, "Found a session: {%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x} for application {%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x}\n", 
		...
		*pcb = 0;
		...
		if (CB_ZTS == cbBlob)
		{
			if (pBlobSrc) // we have something for this parm (as opposed to an intentional NULL)
			{
				...
				{
					int cbLen = strlen((char *)(pBlobSrc));
...
bool FedMessaging::KillSvr()
{
	...
	BOOL fCreated = CreateProcessA(NULL, "KillSvr.exe", NULL, NULL, FALSE, 
	...
	else
	{
		...
		m_pfmSite->OnMessageBox(this, (LPCSTR)lpMsgBuf, "Couldn't run killsvr.exe.", MB_OK | MB_ICONINFORMATION);
		LocalFree( lpMsgBuf );
	}
...
FMSessionDesc::FMSessionDesc( const DPN_APPLICATION_DESC* appDesc)
{
	...
	while(szName)
	{
		if (!strcmp(szName, "GAM"))
		{
			szValue = strtok(NULL, "\n");
			m_strGameName = szValue;
		}
		else if (!strcmp(szName, "PLR"))
		{
			szValue = strtok(NULL, "\n");
			m_nNumPlayers = (short)atoi(szValue);
		}
		else if (!strcmp(szName, "LIM"))
		{
			szValue = strtok(NULL, "\n");
