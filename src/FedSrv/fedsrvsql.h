...
{
public:
	virtual int OnMessageBox(const wchar_t * strText, const wchar_t * strCaption, UINT nType) = 0;
};
...
#define DEF_STMT(NAME, SQL) \
  SQLHSTMT g_hstmt##NAME = AddStatement((SQLWCHAR *) SQL); 
...
#define SQL_STR_PARM(NAME, LENGTH, INOUT) \
  SQLWCHAR NAME [LENGTH + 1] = {AddCol(NAME, SQL_C_CHAR, INOUT, LENGTH + 1)};
...
#define SQL_INT1_PARM(NAME, INOUT) \
  SQL_PARM(SQLWCHAR, NAME, SQL_C_TINYINT, INOUT)
...
#define SQL_BIN_PARM(NAME, LENGTH, INOUT) \
  SQLWCHAR NAME[LENGTH] = {(wchar_t) AddCol(NAME, SQL_C_BINARY, INOUT, sizeof(NAME))};
...
void SqlStrCpy(char * szDest, SQLWCHAR * szSrc, unsigned int cbSrc);
