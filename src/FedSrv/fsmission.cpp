    ...
    m_strStoryText(pszStoryText)
{
  ...
  if (szDesc)
  {
    assert(strlen(szDesc) < sizeof(m_misdef.szDescription));
    Strcpy(m_misdef.szDescription, szDesc);
  }
  else
  {
  ...
  sprintf(szBuff, "%s%s", m_misdef.misparms.strGameName, szRealTeams);
...
void CFSMission::CreateDPGroups(IclusterIGC * pcluster)
{
  ...
  sprintf(szBuff, "%s%s", szDocked, pcluster->GetName());
