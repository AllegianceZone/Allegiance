...
bool IsServerAllowed(const char *ip)
{
	...
	GetModuleFileNameA(NULL, module_file_name, MAX_PATH);
    ...
    GetPrivateProfileStringA(c_szCfgApp, "NumberOfServers", "", 
                                  numservStr, sizeof(numservStr), config_file_name);

    GetPrivateProfileStringA(c_szCfgApp, "FilterType", "", 
	...
	if (strlen(numservStr)>0) {
		...
		for (i=1; i<=numServers; i++) {
			...
			GetPrivateProfileStringA(c_szCfgApp, key, "", 
...
HRESULT LobbyServerSite::OnAppMessage(FedMessaging * pthis, CFMConnection & cnxnFrom, FEDMESSAGE * pfm)
{
  ...
  switch (pfm->fmid)
  {
	...
	case FM_LS_PLAYER_RANK:
	{
		...
		{
			CCssSoap cssSoap(g_pLobbyApp->GetCssServerDomain(), g_pLobbyApp->GetCssClientServicePath(), g_pLobbyApp->GetCssLobbyServicePath(), g_pLobbyApp->GetCssGameDataServicePath());
			rankRetrieved = cssSoap.GetRankForCallsign(
				szCharacterName, 
				&rank,
				&sigma,
				&mu,
		...
		if (g_pLobbyApp->IsCssAuthenticationEnabled() == true)
		{
			...
			CCssSoap cssSoap(g_pLobbyApp->GetCssServerDomain(), g_pLobbyApp->GetCssClientServicePath(), g_pLobbyApp->GetCssLobbyServicePath(), g_pLobbyApp->GetCssGameDataServicePath());
			if (cssSoap.GetUsernameForUsernameOrCallsign((char *)szCharacterName, szUsername, (char *)szReason) == true)
			{
		...
		if ((pfmPlayerRankRequest->characterID >= 17 && pfmPlayerRankRequest->characterID <= 26) || (pfmPlayerRankRequest->characterID >= 39 && pfmPlayerRankRequest->characterID <= 69))
			...
			pfmPlayerRankResponse->fCanCheat = false;
#endif
