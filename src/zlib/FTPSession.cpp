...
protected:
    ...
    virtual bool StartNextFile()
    {
        ...
        if (*m_pszFileList)
        {
            ...
            //
            while (!(m_hFileConnection = FtpOpenFileA(m_hFTPSession, *m_pszFileList, GENERIC_READ, FTP_TRANSFER_TYPE_BINARY | INTERNET_FLAG_RELOAD, 0)))
            {
                //
...
*/
    ...
    void FormatErrorMessage(char *szBuffer, DWORD dwErrorCode)
    {
      ...
      InternetGetLastResponseInfoA(&dummy, szBuffer + strlen(szBuffer), &size);
    }
...
public:
    ...
    virtual bool ConnectToSite(const char * szFTPSite, const char * szDirectory, const char * szUsername, const char * szPassword) 
    {
       ...
       m_hInternetSession = ::InternetOpenA(
       ...
       //
       m_hFTPSession = ::InternetConnectA(
        ...
        if (!FtpSetCurrentDirectoryA(m_hFTPSession, szDirectory))
        {
...
public:
    ...
    virtual bool StartNextFile()
    {
        ...
        if (*m_pszFileList)
        {
            ...
            // Open file for download
            //
            while (!(m_hFileConnection = InternetOpenUrlA(m_hInternetSession, *m_pszFileList, NULL, 0, INTERNET_FLAG_RELOAD | INTERNET_FLAG_NO_CACHE_WRITE, 0))) //Fix memory leak -Imago 8/2/09
            {
                cTries++;
