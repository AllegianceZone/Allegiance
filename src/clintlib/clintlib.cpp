    ...
    else
	{
		if (bStandalonePrivate) {
            ...
            if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, KEY_READ, &hKey)) {
                ::RegQueryValueExA(hKey,"ServerAddress", NULL, NULL, (unsigned char*)&szServer, &cbValue);
                ::RegCloseKey(hKey);
            }
            if (szServer[0] != '\0') {
...
*/
    ...
    if (m_fmLobby.IsConnected())
    {
        ...
        sprintf(szEncryptionKey, CL_LOGON_KEY, dwTime, m_ci.szName);
...
HRESULT BaseClient::ConnectToClub(ConnectInfo * pci) // pci is NULL if relogging in
{
    ...
    if (m_fmClub.IsConnected())
    {
        ...
        strcpy(m_szClubCharName, pci->szName);
        SendClubMessages();
    }
    m_cUnansweredPings = 0;
    m_serverOffsetValidF = false;
...
bool BaseClient::ResetStaticData(const char * szIGCStaticFile, ImissionIGC** ppStaticIGC, Time tNow, bool bEncrypt)
{
    ...
    // copy the core name
    strcpy(m_szIGCStaticFile, szIGCStaticFile);
...
void CfgInfo::Load(const char * szConfig)
{
	...
	GetPrivateProfileStringA(c_szCfgApp, "TrainingURL", "http://allegiancezone.com/#/Training",
                                   szStr, sizeof(szStr), szConfig);
    strTrainingURL = szStr;

	GetPrivateProfileStringA(c_szCfgApp, "PassportUpdateURL", "http://allegiancezone.com",
                                   szStr, sizeof(szStr), szConfig);
    strPassportUpdateURL = szStr;

	GetPrivateProfileStringA(c_szCfgApp, "ZoneAuthGUID", "{00000000-0000-0000-C000-000000000046}",
                                   szStr, sizeof(szStr), szConfig);
	...
	GetPrivateProfileStringA(c_szCfgApp, "FilelistCRC", "0",
                                   szStr, sizeof(szStr), szConfig);
    _strupr(szStr);
	...
	GetPrivateProfileStringA(c_szCfgApp, "FilelistSize", "0",
                                   szStr, sizeof(szStr), szConfig);
	...
	GetPrivateProfileStringA(c_szCfgApp, "FilelistSite", "",
                                   szStr, sizeof(szStr), szConfig);
    strFilelistSite = szStr;
    
	GetPrivateProfileStringA(c_szCfgApp, "FilelistDirectory", "",
                                   szStr, sizeof(szStr), szConfig);
    strFilelistDirectory = szStr;

	GetPrivateProfileStringA(c_szCfgApp, "ClubMessageCRC", "0",
                                    szStr, sizeof(szStr), szConfig);
    _strupr(szStr);
    crcClubMessageFile = UTL::hextoi(szStr);

	GetPrivateProfileStringA(c_szCfgApp, "PublicMessageCRC", "0",
                                    szStr, sizeof(szStr), szConfig);
    _strupr(szStr);
    crcPublicMessageFile = UTL::hextoi(szStr);

	GetPrivateProfileStringA(c_szCfgApp, "UsePassport", "0",
                                   szStr, sizeof(szStr), szConfig);
    bUsePassport = atoi(szStr) != 0;

    // mdvalley: get lobby port
	GetPrivateProfileStringA(c_szCfgApp, "LobbyClientPort", "2302",
                                   szStr, sizeof(szStr), szConfig);
	dwLobbyPort = atoi(szStr);
    // Imago 9/14
	GetPrivateProfileStringA(c_szCfgApp, "ClubClientPort", "2304",
                                   szStr, sizeof(szStr), szConfig);
